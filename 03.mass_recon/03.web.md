...
### 3.2. HTTP/HTTPS services

Default ports for web services:
```
...
6. HTTProbe
```
$ cat $domain.txt | httprobe -p xlarge -c 10
```

7 . Masscan/Nmap + Aquatone
```
$ sudo masscan -iL IPs.txt -p$ports -oX scan.xml --rate 100
$ nmap -Pn -n -iL IPs.txt -p$ports -oX scan.xml
