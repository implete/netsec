...
7. HTTProbe
```
$ cat $domain"_alive_DNS.txt" | httprobe -p xlarge -c 10 > $domain"_alive_DNS_web.txt"
$ cat $domain"_alive_IPs.txt" | httprobe -p xlarge -c 10 > $domain"_alive_IPs_web.txt"
```
...
$ nmap -Pn -n -iL $domain"_alive_IPs.txt" -p$ports -oG - > $domain"_alive_IP_nmap.txt"
```
